    ...
    if (self) {
        ...
        for (int i = 0;  i < arr.count; i ++) {
            ...
            for (NSString *key in dic.allKeys) {
                ...
                if ([key isEqualToString:@"lngLatitude"]) {
                    self.lngLatitude = dic[key];
                    continue;
                }
                if ([key isEqualToString:arr[i]]) {
...
+ (instancetype) myDataModelWithDictionary:(NSDictionary *)dic {
    return [[SearchModelJS alloc] initWithDic:dic];
}
